...
/*   Updated: 2023/09/13 17:22:11 by svan-de-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	read_normal_map(t_normal_map *normal_map, char *file_name)
{
	...
	free(name_path);
	if (fd == -1)
		return (ft_putstr_fd(FDNM_ERR, 2), 1);
	...
	normal_map->map = malloc(sizeof(t_vec)
			* normal_map->width * normal_map->height);
	if (!normal_map->map)
		return (error_allocation(), close(fd), 1);
	...
	while (i < NB_MAP)
	{
		if (good_maps[i])
			if (read_normal_map(rt->normal_maps + i, good_file(i)))
				(free_maps(rt->normal_maps, good_maps),
