...
/*   Updated: 2023/09/11 12:53:38 by svan-de-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void handle_zoom(t_cam *cam, int mouse_dir_y) {
    cam->system.origin = add(cam->system.origin, scale(cam->dir, -mouse_dir_y / 30.0));
}
...
void rotate_camera(t_cam *cam, int mouse_dir_x, int mouse_dir_y) {
    ...
    cam->system.origin = get_ray_point((t_ray){cam->look_at, cam->dir}, -cam->radius);
}
...
int handle_mouse_down(int button, int x, int y, t_rt *rt) {
    ...
    cam->look_at = add(cam->system.origin,
