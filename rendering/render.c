...
t_vec	to_world_coordinates(t_cam *cam, double x, double y)
{
	...
	x = (2 * x / WIDTH - 1) * tan_view_field;
	y = (1 - 2 * y / HEIGHT) * tan_view_field * ((float)HEIGHT / WIDTH);
...
void	set_cam_ray(t_world *world, t_ray *cam_ray, int x, int y)
{
	...
	blur = randf() * CAM_FUZZ - CAM_FUZZ / 2;
...
t_rgb	get_pixel_color(t_rt *rt, int x, int y, int curr_frame)
{
	...
	rt->pixel_buff[y * WIDTH + x] += pixel_color;
	return (rt->pixel_buff[y * WIDTH + x] / curr_frame);
}
